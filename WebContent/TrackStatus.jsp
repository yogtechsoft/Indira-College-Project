							...
							<select class="form-control" id="srNo" name="srNo">
								<option>Select</option>
									<%
									 ...
									 ResultSet contractorName=null;
									if(roleId == 2){
										 contractorName = DatabaseConnection.getResultFromSqlQuery("SELECT id,institute_name FROM `tbl_indent_save_details` WHERE status='Waiting for Approval' || status='Approved'");

									}else if(roleId ==3){
										 contractorName = DatabaseConnection.getResultFromSqlQuery("SELECT id,institute_name FROM `tbl_indent_save_details` WHERE status='Approved' || status='Waiting for Approval'");
									}
									else if(roleId ==4){
										 contractorName = DatabaseConnection.getResultFromSqlQuery("SELECT id,institute_name FROM `tbl_indent_save_details` WHERE status='Approved' || status='Waiting for Approval'");
									}
			...
			<div class="row">
				<div class="col-md-12 col-sm-12 col-xs-12">
					<div class="panel panel-success">
						<div class="panel-heading">Track Indent Status</div>
