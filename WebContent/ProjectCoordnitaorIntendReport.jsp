...
<form role="form" action="UpdateApplicationStatusProjectCordniator" method="post">
						...
						<label>Select Institute Name</label> 
							<select class="form-control" id="srNo" name="srNo">
								<option value="0">Select</option>
									...
									ResultSet contractorName = DatabaseConnection.getResultFromSqlQuery("SELECT id,institute_name FROM `tbl_project_cordniator_application_received_details` WHERE role_id="+roleId+" and status='"+status+"'");
	...
	<script>
     ...
     $("#downloadPDF").click(function(){
    	 var srNo = $('#srNo').val();
    	    window.location.href = 'GenerateIndentReportDirector?srNo=' + srNo;
     });
	...
	$("#srNo").change(function(){
		$.ajax({
			url : 'fetchIndentDetailsProjectCoordnitor',
			data : {
				srNo:$("#srNo").val()
			},
			success : function(responseText) { 
