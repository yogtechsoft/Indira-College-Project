...
<form role="form" action="UpdateApplicationStatusDirector" method="post">
						...
						<label>Select Institute Name</label> 
							<select class="form-control" id="srNo" name="srNo">
								<option value="0">Select</option>
									...
									ResultSet contractorName = DatabaseConnection.getResultFromSqlQuery("SELECT id,institute_name FROM `tbl_director_department_application_received_dewtails` WHERE role_id="+roleId+" and status='"+status+"'");
	...
	$("#srNo").change(function(){
		$.ajax({
			url : 'fetchIndentDetailsProjectDm',
			data : {
				srNo:$("#srNo").val()
			},
			success : function(responseText) { 
