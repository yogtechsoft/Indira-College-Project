		...
		try {
	        ...
	        if(total == 100) {
	        	int update = DatabaseConnection.insertUpdateFromSqlQuery
	        			("update tblnewconstructionworkdetails set totalWorkComplete='"+String.valueOf(total)+"', totalPendingWork='"+String.valueOf(0)+"' WHERE  workDetails='"+contractorWork+"' and contractorId="+contractorDetails);
	        			if(update == 1) {
	        				String str="Record Update Succrssfully";				
	        				//insert siteInspection Details
	        				int i = DatabaseConnection.insertUpdateFromSqlQuery("insert into tblsiteinspectiondetails(siteWorkDetails,campus,building,floor,startDate,endDate,dailyTaskDetails,contractorId,todayDate,modifiedBy,file_name,file_data) "
	        	...
	        	int update = DatabaseConnection.insertUpdateFromSqlQuery
	        			("update tblnewconstructionworkdetails set totalWorkComplete='"+String.valueOf(total)+"' WHERE  workDetails='"+contractorWork+"' and contractorId="+contractorDetails);
	        			if(update == 1) {
	        				String str="Record Update Succrssfully";				
	        				//insert siteInspection Details
	        				int i = DatabaseConnection.insertUpdateFromSqlQuery("insert into tblsiteinspectiondetails(siteWorkDetails,campus,building,floor,startDate,endDate,dailyTaskDetails,contractorId,todayDate,modifiedBy,file_name,file_data) "
