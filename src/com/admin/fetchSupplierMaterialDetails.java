...
@WebServlet("/fetchSupplierMaterialDetails")
public class fetchSupplierMaterialDetails extends  HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet captchResultSet = DatabaseConnection.getResultFromSqlQuery("SELECT * FROM `tblmaterialdetails` WHERE customerId="+customerId);
			while (captchResultSet.next()) {
				...
				ResultSet fetchSupplierDetails = DatabaseConnection.getResultFromSqlQuery
						("SELECT SUM(totalAmount) FROM `tblmaterialdetails` WHERE customerId="+customerId);
