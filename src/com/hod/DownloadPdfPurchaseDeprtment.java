...
@WebServlet("/DownloadPdfPurchaseDeprtment")
public class DownloadPdfPurchaseDeprtment extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
			...
			ResultSet captchResultSet = DatabaseConnection.getResultFromSqlQuery("SELECT filename,filedata FROM `tbl_purchase_department_application_received_details` where id="+fileId);

			if (captchResultSet.next()) {
				String fileName = captchResultSet.getString("filename");
	            Blob blob = captchResultSet.getBlob("filedata");
