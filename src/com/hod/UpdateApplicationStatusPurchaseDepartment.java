			...
			if(action.equals("1")) {
				...
				int i = statement.executeUpdate("UPDATE tbl_purchase_department_application_received_details set status='Approved',hod_status_remark='Approved',hod_approved_date ='"+dateDetails+"' where id="+srNO);
					...
					int addCustomer = DatabaseConnection.insertUpdateFromSqlQuery(
							...
							+ "values('" +InstituteName+ "','" + indenterName + "','" + department + "','" + date + "','" + discription + "','" + materialRequired + "','"+ quantity + "','" + reasonWork + "','" + specificAgency + "','" + indentValue + "','" + deliveryRequired + "','"+ workCompletion + "','" + previousRef +"','"+ remark +"','"+status+"',"+userId+","+role+",'"+asas+"','"+dateDetails+"','"+fileName+"','"+fileData+"')");
					if(addCustomer>0) {
