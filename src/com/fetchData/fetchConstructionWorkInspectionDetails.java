...
@WebServlet("/fetchConstructionWorkInspectionDetails")
public class fetchConstructionWorkInspectionDetails extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			("SELECT campus,building,floor FROM tblnewconstructionworkdetails WHERE workDetails='"+contractorWorkDetails+"' and contractorId="+customerSetails);
			while (captchResultSet.next()) {
				ConstuctionInspectionModel st=new ConstuctionInspectionModel();
				st.setCampus(captchResultSet.getString(1));
				st.setBuilding(captchResultSet.getString(2));
				st.setFloor(captchResultSet.getString(3));
				getDetails.add(st);
