...
@WebServlet("/fetchMaintainceWorkName")
public class fetchMaintainceWorkName extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			("SELECT workDetails FROM tblmaintanceconstructiondetails WHERE contractorId="+contractorId);
			while (captchResultSet.next()) {
				ConstructionSiteModel st=new ConstructionSiteModel();
				st.setMaintainceConstructionWorkName(captchResultSet.getString(1));
				getDetails.add(st);
