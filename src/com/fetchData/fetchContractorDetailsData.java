...
@WebServlet("/fetchContractorDetailsData")
public class fetchContractorDetailsData extends HttpServlet {
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {

		try {
			String customerId = request.getParameter("customerName");
			List<ContractorCustomerModel> getDetails=new ArrayList<ContractorCustomerModel>();
			ResultSet captchResultSet = DatabaseConnection.getResultFromSqlQuery("SELECT * FROM `tbl_save_labour_contractor_details` WHERE customerId="+customerId);
			while (captchResultSet.next()) {
				ContractorCustomerModel st=new ContractorCustomerModel();
				...
				ResultSet fetchSupplierDetails = DatabaseConnection.getResultFromSqlQuery
						("SELECT SUM(total_amount) FROM `tbl_save_labour_contractor_details` WHERE customerId="+customerId);
